from collective.dexteritydiff.choicediff import ChoiceDiff
from collective.dexteritydiff.testing import (INTEGRATION_TESTING,
    TEST_CONTENT_TYPE_ID, VOCABULARY_TUPLES, VOCABULARY)
from collective.dexteritydiff.utils import title_or_value
...
class ChoiceDiffTestCase(unittest.TestCase):
    ...
    def test_should_diff_choice_field(self):
        self._test_diff_choice(
            VOCABULARY_TUPLES[0][0], VOCABULARY_TUPLES[0][0], True)
        self._test_diff_choice(
            VOCABULARY_TUPLES[0][0], VOCABULARY_TUPLES[1][0], False)
        self._test_diff_choice(
            VOCABULARY_TUPLES[1][0], VOCABULARY_TUPLES[0][0], False)
        self._test_diff_choice(
            VOCABULARY_TUPLES[1][0], VOCABULARY_TUPLES[1][0], True)
